...
export class InitSchema1776065349582 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_6a151432891697dc7337aadd27" ON "promo_codes" ("expires_at") `,
    );
    await queryRunner.query(
      `CREATE TABLE "activations" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "promo_code_id" uuid NOT NULL, "email" citext NOT NULL, "activated_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "PK_db9ffdd659f4ac699248030b596" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_ee1e3b5de9e79d35880669655d" ON "activations" ("promo_code_id") `,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX "uq_activation_promo_email" ON "activations" ("promo_code_id", "email") `,
    );
    await queryRunner.query(
      `ALTER TABLE "activations" ADD CONSTRAINT "FK_ee1e3b5de9e79d35880669655db" FOREIGN KEY ("promo_code_id") REFERENCES "promo_codes"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
  }
